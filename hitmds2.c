...
int data_alloc(void)
{
  ...
  matsize = 0;
  for (i = 0; i < abs(pattern_length); ++i)
    matsize += abs(pattern_length) - 1;
  matsize = matsize >> 1;

  /* upper triangle matrix */
  tdim = sizeof(FLT) * (1 + matsize);

  fprintf(stderr, "%d %d %d \n",(abs(pattern_length)-1) * 
    abs(pattern_length), matsize,tdim);
...
char s1[20];
  ...
  fprintf(stderr, "%d %d \n", pattern_length, pattern_dimension);
  if(!data_alloc())
  ...
  for(line = 0; line < pattern_length; ++line) {
    ...
    for(column = 0; column < pattern_dimension; ++column) {
      ...
      if (1 != fread(&dtmp, sizeof(double), 1, file)){

        char panistr[100];
        sprintf(panistr, "%g data_stdin(): Trouble reading data in line #%d/%d, column #%d/%d !", D(pattern_distmat, line, column-1), line+1+off, pattern_length, column+1, pattern_dimension);
        error(panistr);
      }
      else if(matrix_input) {
        D(pattern_distmat, line, column) = (FLT)dtmp;
        //fprintf(stderr, "%f ", (double)((FLT)dtmp));
        //if (dtmp > max_dist)
        //    max_dist = dtmp;
      }
    }
    ...
    for(column = 0; column < target_dim; column++) {
      ...
      if(tdim > 0) { /* read from input */
        ...
        if(1 != scanf(SCANFMT, points[line]+column)) {
          char panistr[100];
          sprintf(panistr, "data_stdin(): Trouble 2 reading pre-inited points in line #%d/%d, column #%d/%d !", line+off, pattern_length, column+1, target_dim);
          error(panistr);
        }
      }
      else { 
  ...
  */
  ...
  for (i = 0; i < pattern_length; ++i) {
    for (j = i; j < pattern_length; ++j) {
      if (dist(target_dim,points[i],points[j]) > max_dist)
          max_dist = dist(target_dim,points[i],points[j]);
    }    
  }

  for(j = 0; j < target_dim; j++)
    for(i = 0; i < pattern_length; i++)
      points[i][j] *= 1 / max_dist;
}
...
/* model output */
void mds_stdout(const char *filename) 
{
  ...
  for(i = 0; i < pattern_length; i++) {
    ...
    for(j = 0; j < target_dim; j++) fprintf(fp, "%g ", points[i][j]);
...
void main_bye(const char *filename) {
  
  fprintf(stderr, "# corr(D,d): %g\n", 1./sqrt(corr_2()+1));
...
int main (int argc, char *argv [])
{
  ...
  pattern_length = - atoi(argv[6]);
  pattern_dimension = -1;
  target_dim = -2;

  data_stdin(argv[5]);  /* calls data_alloc */
